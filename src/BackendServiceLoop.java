 ...
 */
public class BackendServiceLoop extends HttpServlet {
    ...
    public BackendServiceLoop(int port, boolean isLast) {
        this.port = port;
        this.isLast = isLast;
...
//                connectToDatabase(writer);
    ...
    private void connectToDatabase(PrintWriter writer, String name ) throws IOException {
        URL url = new URL("http://localhost:" + 8990 + "/bt2_end?name="+name);
        writer.println("--" + IOUtils.toString(url.openConnection().getInputStream(), "utf-8"));
    }

    public static void main(String[] args) throws Exception {
//        BackendServiceLoop.start("backend_receive_and_callback", 8988, false).join();
        Server server1 = BackendServiceLoop.start("bt1_end", 8988, true/*end of bt*/);
        Server server2 = BackendServiceLoop.start("bt2_start", 8989, false /*not end of bt*/);
        server1.join();
        server2.join();
    }
    ...
    public static Server start(String servletName, int port, boolean isLast) throws Exception {
        ...
        BackendServiceLoop backendService = new BackendServiceLoop(port, isLast);
    ...
    public class SetSleepTime extends HttpServlet {
        protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            BackendServiceLoop.this.sleepTime = Long.parseLong(request.getParameter("s"));
        }
    }

    public class SetRequestCount extends HttpServlet {
        protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            BackendServiceLoop.this.requestCount = Integer.parseInt(request.getParameter("c"));
        }
    }
}
